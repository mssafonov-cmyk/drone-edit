...
## ⚡ Режим экономии токенов (ОБЯЗАТЕЛЕН с 2026-09-06, интервал 4 часа)
Цель: прогон ≤ ~1/10 стоимости первого. Порядок:
1. `git pull`, затем **`python3 charter_watch/run_check.py`** — фетчеры из `sources/` сами
   обновляют snapshot по известным источникам. Читать только вывод скрипта.
2. Вручную (curl/WebFetch) проверять ТОЛЬКО: (а) лодки из `state/watchlist.json → priority`,
   у источника которых ещё нет фетчера; (б) источники, у которых фетчер упал (`fail:`).
   Лимит: **≤ 12 сетевых запросов вручную за прогон, без субагентов.**
3. Каждый работающий ручной приём (URL, параметры, разбор ответа) в тот же прогон
   оформить как фетчер `sources/<name>.py` (контракт — в `run_check.py`), чтобы в
   следующий раз он шёл скриптом. Это главный механизм экономии.
4. Discovery новых лодок (поиск по 9 окнам на YachtReal/CY24/Boataround/12knots) —
   только если `watchlist.last_discovery` старше 24 часов; обновить `last_discovery`.
5. `python3 charter_watch/compare.py <prev> state/snapshot.json` → события только по скрипту.
6. Не открывать `snapshot.json` целиком в контекст; правки — через python-однострочники.
7. Финальное сообщение ≤ 10 строк. Никаких пересказов истории.
8. Источники, стабильно закрытые антиботом (charter.boats/api.charter.boats, Dream Yacht
   Charter 403, nausys.com таймаут), вручную пробовать не чаще 1 раза в сутки (отметка в
   runlog); в остальные прогоны — не тратить запросы. Asia Marine: страница статическая,
   дат/цен нет — не открывать.

## 🔒 Удерживаемая лодка (held) — с 2026-09-12
`state/watchlist.json → held`: **KARYSTA (Lagoon 52 F, DYC), окно 09–16.11.2026, опция НЕ оплачена.**
Логика по ней ИНВЕРТИРОВАНА относительно остальных лодок:
- Занятое окно (`booked` у источников) — НОРМА, ничего не делать, в финальной строке писать
  `hold ok`.
- Окно снова доступно → `compare.py` выдаёт событие **HOLD_LOST** (exit 0). Это тревога, а не
  хорошая новость: опция слетела, лодку могут перехватить. Слать `PushNotification`
  (status=proactive) немедленно.
- `karysta` исключена из обычных событий (NEW/RELEASED/PRICE DROP/BETTER OPTION) — только HOLD_LOST.
- Снять удержание (после оплаты или отказа) — удалить блок `held` из `watchlist.json`.
